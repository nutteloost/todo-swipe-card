...
`}
    `}St(t,e){if(!this.l?.entities)return;const o=[...this.l.entities],i=t+e;if(i<0||i>=o.length)return;[o[t],o[i]]=[o[i],o[t]],this.dt.has(t)&&(this.dt.delete(t),this.dt.add(i)),this.dt.has(i)&&(this.dt.delete(i),this.dt.add(t));const n={...this.l,entities:o};this.l=n,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:n}})),this.requestUpdate()}Mt(t){this.dt.has(t)?this.dt.delete(t):(this.dt.clear(),this.dt.add(t)),this.requestUpdate()}At(t){this.ct=t,this.requestUpdate(),setTimeout(()=>{this.ct="normal",this.requestUpdate()},"blocked"===t?1e3:500)}Nt(t=-1){if(!this.hass)return[];const e=Object.keys(this.hass.states).filter(t=>t.startsWith("todo.")&&this.hass.states[t]),o=(this.l.entities||[]).map((e,o)=>o===t?null:this.p(e)).filter(t=>t&&""!==t.trim());return e.filter(t=>!o.includes(t))}Ot(t){const e=this.p(t);if(!e||""===e.trim())return{displayName:"Empty Entity",friendlyName:""};const o=this.hass?.states?.[e],i=o?.attributes?.friendly_name||e.split(".").pop().replace(/_/g," ");return{displayName:i,friendlyName:i}}It(t){if(!this.l||!this.hass)return;const e=t.target,o=void 0!==e.checked?e.checked:e.value,i=e.configValue||e.getAttribute("data-config-value");if(i){const t=this.ft({...this.l,[i]:o});this.l=t,this.Lt(t)}}Lt(t){this.Ht&&clearTimeout(this.Ht),this.Ht=setTimeout(()=>{const e=this.ft(t);this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e}}))},150)}jt(t){if(!this.l)return;const e=parseInt(t.target.value);if(!isNaN(e)&&e>=0){const t=this.ft({...this.l,card_spacing:e});this.l=t,this.Lt(t)}}lt(t){if(t&&(t.preventDefault(),t.stopPropagation()),!this.l)return;const e=Array.isArray(this.l.entities)?[...this.l.entities]:[];if(e.length>0&&(""===e[e.length-1]||"object"==typeof e[e.length-1]&&""===e[e.length-1].entity))return void this.At("blocked");e.push({entity:""});const o={...this.l,entities:e};this.l=o,this.At("success"),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:o},bubbles:!0,composed:!0})),setTimeout(()=>{this.requestUpdate()},0)}Vt(t){if(!this.l||!Array.isArray(this.l.entities))return;const e=[...this.l.entities];e.splice(t,1),this.dt.delete(t);const o=new Set;this.dt.forEach(e=>{e>t?o.add(e-1):e<t&&o.add(e)}),this.dt=o;const i={...this.l,entities:e};this.l=i,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:i}})),this.requestUpdate()}Ft(t){const e=parseInt(t.target.getAttribute("data-index"));if(isNaN(e))return;const o=t.detail?.value||t.target.value||"",i=[...this.l.entities],n=i[e];i[e]="object"==typeof n?{...n,entity:o}:{entity:o};const r={...this.l,entities:i};this.l=r,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:r}})),this.requestUpdate()}Jt(t){const e=parseInt(t.target.getAttribute("data-index"));if(isNaN(e))return;const o=t.target.value||"none",i=[...this.l.entities],n=i[e];i[e]="string"==typeof n?{entity:n,display_order:o}:{...n,display_order:o};const r={...this.l,entities:i};this.l=r,this.Lt(r)}Bt(t){const e=parseInt(t.target.getAttribute("data-index"));if(isNaN(e))return;const o=t.target.value||"",i=[...this.l.entities],n=i[e];if("string"==typeof n){const t={entity:n};o&&(t.background_image=o),i[e]=t}else if(o)i[e]={...n,background_image:o};else{const t={...n};delete t.background_image,i[e]=t}const r={...this.l,entities:i};this.l=r,this.Lt(r)}Rt(t){const e=parseInt(t.target.getAttribute("data-index"));if(isNaN(e))return;const o=t.target.checked,i=[...this.l.entities],n=i[e];i[e]="string"==typeof n?{entity:n,show_title:o}:{...n,show_title:o};const r={...this.l,entities:i};this.l=r,this.Lt(r)}Zt(t){const e=parseInt(t.target.getAttribute("data-index"));if(isNaN(e))return;const o=t.target.value||"",i=[...this.l.entities],n=i[e];if("string"==typeof n){const t={entity:n};o&&(t.title=o),i[e]=t}else if(o)i[e]={...n,title:o};else{const t={...n};delete t.title,i[e]=t}const r={...this.l,entities:i};this.l=r,this.Lt(r)}qt(t){const e=parseInt(t.target.getAttribute("data-index"));if(isNaN(e))return;const o=t.target.value||"",i=[...this.l.entities],n=i[e];if("string"==typeof n){const t={entity:n};o&&(t.icon=o),i[e]=t}else if(o)i[e]={...n,icon:o};else{const t={...n};delete t.icon,i[e]=t}const r={...this.l,entities:i};this.l=r,this.Lt(r)}Pt(t){const e=this.l.entities[t];return"string"==typeof e?{entity:e,display_order:"none",show_title:!1,title:"",background_image:""}:{entity:e?.entity||"",display_order:e?.display_order||"none",show_title:e?.show_title||!1,title:e?.title||"",background_image:e?.background_image||"",icon:e?.icon||""}}Xt(){return[{value:e,label:"Default"},{value:o,label:"Alphabetical A-Z"},{value:i,label:"Alphabetical Z-A"},{value:n,label:"Due Date (Earliest First)"},{value:r,label:"Due Date (Latest First)"}]}Ut(t,e){t.stopPropagation(),this.Mt(e)}Gt(t,e){t.stopPropagation(),e()}Kt(t,e){"Enter"!==t.key&&" "!==t.key||(t.preventDefault(),t.stopPropagation(),this.Mt(e))}Wt(t){t.stopPropagation(),t.preventDefault()}render(){if(!this.hass||!this.l)return d`<div>Loading...</div>`;const e=Array.isArray(this.l.entities)?this.l.entities:[];return JSON.stringify(this.l),d`
        ...
        <div class="section">
          ...
          <div class="card-list">
            ${0===e.length?d`<div class="no-cards">
                  No todo lists added yet. Click "+ ADD TODO LIST" below to add your first list.
                </div>`:e.map((t,o)=>{const i=this.Ot(t),n=this.dt.has(o),r=this.Pt(o);return d`
                    <div
                    ...
                    >
                      ...
                      <div class="card-actions" @click=${this.Wt}>
                        <ha-icon-button
                          label="Move Up"
                          ?disabled=${0===o}
                          path="M7,15L12,10L17,15H7Z"
                          @click=${t=>this.Gt(t,()=>this.St(o,-1))}
                        ></ha-icon-button>
                        <ha-icon-button
                          label="Move Down"
                    ...
                    ${n?d`
                          <div class="expanded-content">
                            <ha-entity-picker
                              ...
                              .value=${r.entity}
                              .includeDomains=${["todo"]}
                              .includeEntities=${this.Nt(o)}
                              data-index=${o}
                              @value-changed=${this.Ft}
                              allow-custom-entity
                              label="Todo Entity"
                            ></ha-entity-picker>
          ...
          <div class="add-entity-button">
            <button
              class="add-todo-button ${"normal"!==this.ct?this.ct:""}"
              @click=${t=>this.lt(t)}
            >
